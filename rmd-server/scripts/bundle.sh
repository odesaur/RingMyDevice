...
ZIPNAME="rmd-server-${REF}.zip"
HASHNAME="rmd-server-${REF}.zip.sha256sum"

OUTDIR="$PWD"
BUILDDIR=/tmp/rmdbuild
...
git clone --quiet "${GIT_CLONE_URL:-https://github.com/odesaur/RingMyDevice.git}" rmd-server
pushd rmd-server/rmd-server
...
for ARCH in "amd64" "arm" "arm64"; do
    ...
    go build -ldflags="-w -s -buildid=" -trimpath -o "rmd-server-$GOARCH"
done
